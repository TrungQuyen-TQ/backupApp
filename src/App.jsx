...
const handleCheckVersion = async (log) => {
  try {
    ...
    if (result.success) {
      ...
      alert(`🚀 Phiên bản SQL: ${result.version}\n📊 Quét được: ${tableCount} bảng.`);
      
      // Nếu bạn muốn hiện Modal danh sách bảng ngay khi bấm CHECK:
      setBackupStats({ stats: { rowCounts: result.counts }, fileName: "Thông tin hiện tại" });
      setShowBackupModal(true);
    } else {
      alert(`❌ Lỗi: ${result.error}`);
    }
    ...
    <Box sx={{ bgcolor: '#eaeff1', minHeight: '100vh', p: 3, display: 'flex', gap: 2 }}>
      ...
      <Modal open={showBackupModal} onClose={() => setShowBackupModal(false)}>
        <Box sx={{
          ...
          width: 450, bgcolor: 'background.paper', borderRadius: '12px', boxShadow: 24, p: 4,
        }}>
          <Typography variant="h6" component="h2" sx={{ mb: 2, color: '#1976d2', fontWeight: 700 }}>
          ...
          <Box sx={{ 
            bgcolor: '#f5f5f5', p: 2, borderRadius: '8px', 
            maxHeight: 300, overflowY: 'auto', border: '1px solid #ddd' 
          }}>
            {backupStats && Object.entries(backupStats.stats.rowCounts).map(([table, count]) => (
              <Box key={table} sx={{ 
                display: 'flex', justifyContent: 'space-between', 
                borderBottom: '1px solid #ddd', py: 0.8 
              }}>
                <Typography variant="body2" sx={{ textTransform: 'capitalize' }}>{table}</Typography>
                <Typography variant="body2" sx={{ fontWeight: 700, color: '#2e7d32' }}>{count} dòng</Typography>
              </Box>
            ))}
          </Box>

          <Button 
            variant="contained" fullWidth sx={{ mt: 3, borderRadius: '8px' }} 
