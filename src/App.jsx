...
function App() {
  ...
  const handleTestConnection = async () => {
    ...
    console.log("Kết quả test kết nối:", formData);
    const newLog = {
      ...formData,
      id: Date.now(),
  ...
  const handleBackupSpecificDb = async (log) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(`❌ Lỗi [${log.database}]: ${result.error}`);
      }
    } catch (err) {
  ...
  const handleOpenBackupConfig = async (log) => {
    ...
    try {
      ...
      else alert("Không thể lấy danh sách DB: " + result.error);
    } catch (error) {
  ...
  return (
    <Box sx={{ bgcolor: "#eaeff1", minHeight: "100vh", p: 3, display: "flex", gap: 2 }}>
      ...
      <Modal open={showInputDbModal} onClose={() => setShowInputDbModal(false)}>
        <Box sx={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 450, bgcolor: "background.paper", borderRadius: "12px", p: 4 }}>
          <Typography variant="h6" sx={{ mb: 2, fontWeight: "bold", color: "#1565c0", display: 'flex', alignItems: 'center' }}>
          ...
          {isFetchingDbs ? (
            ...
            <FormControl sx={{ width: '100%', mt: 1 }}>
              ...
              <Select multiple value={selectedDbs} onChange={handleSelectChange} input={<OutlinedInput label="Danh sách Database" />} renderValue={(selected) => (
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                  {selected.map((value) => <Chip key={value} label={value} size="small" color="primary" />)}
                </Box>
              )} MenuProps={MenuProps}>
                {dbList.length === 0 ? <MenuItem disabled><em>Không có dữ liệu</em></MenuItem> : dbList.map((name) => (
                  <MenuItem key={name} value={name} style={getStyles(name, selectedDbs, theme)}>{name}</MenuItem>
                ))}
              </Select>
            </FormControl>
          )}
          <Box sx={{ mt: 4, display: "flex", gap: 1 }}>
            <Button fullWidth variant="outlined" onClick={() => setShowInputDbModal(false)}>HỦY</Button>
            <Button fullWidth variant="contained" disabled={selectedDbs.length === 0 || isFetchingDbs} onClick={handleBackupMultipleDbs}>
      ...
      <Modal open={showBackupModal} onClose={() => setShowBackupModal(false)}>
        <Box sx={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 450, bgcolor: "background.paper", borderRadius: "12px", p: 4 }}>
          <Typography variant="h6" sx={{ mb: 2, color: "#1976d2", fontWeight: 700 }}>✅ Hoàn tất sao lưu</Typography>
          <Divider sx={{ mb: 2 }} />
          <Typography variant="body2"><b>File:</b> {backupStats?.fileName}</Typography>
          <Box sx={{ bgcolor: "#f5f5f5", p: 2, borderRadius: "8px", maxHeight: 200, overflowY: "auto", mt: 2 }}>
            {backupStats && Object.entries(backupStats.stats.rowCounts).map(([table, count]) => (
              <Box key={table} sx={{ display: "flex", justifyContent: "space-between", borderBottom: "1px solid #ddd", py: 0.5 }}>
                <Typography variant="caption">{table}</Typography>
                <Typography variant="caption" sx={{ fontWeight: 700, color: "#2e7d32" }}>{count} dòng</Typography>
              </Box>
            ))}
          </Box>
          <Button variant="contained" fullWidth sx={{ mt: 3 }} onClick={() => setShowBackupModal(false)}>Đóng</Button>
        </Box>
      </Modal>

      {/* CỘT TRÁI */}
      <Paper elevation={2} sx={{ width: "100%", maxWidth: 500, borderRadius: 2, overflow: "hidden", display: 'flex', flexDirection: 'column' }}>
      ...
      {/* CỘT PHẢI */}
      <Paper elevation={2} sx={{ flex: 1, borderRadius: 2, p: 2, maxHeight: "100vh", overflowY: "auto" }}>
        <Typography variant="h6" sx={{ mb: 2, fontWeight: "bold" }}>Lịch sử kết nối</Typography>
        <Divider />
        <List sx={{ px: 1 }}>
          {connectionLogs.map((log) => (
            <ListItem key={log.id} sx={{ mb: 2, borderRadius: "12px", border: "1px solid #e0e4e8", p: 2, flexDirection: "column", alignItems: "stretch" }}>
              <Box sx={{ display: "flex", justifyContent: "space-between", mb: 1 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                  <Box sx={{ p: 1, borderRadius: "8px", bgcolor: log.success ? "#e8f5e9" : "#ffebee" }}>
                    {log.success ? <CheckCircleIcon color="success" /> : <ErrorIcon color="error" />}
                  </Box>
                  <Box>
                    <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>{log.server}</Typography>
                    <Typography variant="caption" sx={{ color: "#707880" }}>{log.time}</Typography>
                  </Box>
                </Box>
                <IconButton size="small" onClick={() => handleDeleteLog(log.id)} sx={{ color: "#d32f2f" }}><DeleteIcon fontSize="small" /></IconButton>
              </Box>
              <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 1, mt: 1 }}>
                <Button variant="outlined" size="small" onClick={() => handleCheckVersion(log)}>CHECK</Button>
                <Button variant="contained" size="small" disabled={backingUpId !== null} onClick={() => handleOpenBackupConfig(log)} sx={{ bgcolor: log.success ? "#7b1fa2" : "#b0bec5" }}>
