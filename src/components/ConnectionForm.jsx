  ...
  if (options) {
    return (
      <FormControl fullWidth size="small" sx={{ mb: 2.5 }}>
        ...
        <Select labelId={`${name}-label`} id={`${name}-select`} label={label} name={name} value={value} onChange={onChange}>
          {options.map((opt) => (
...
export const ConnectionForm = ({ formData, setFormData, onConnectSuccess }) => {
  ...
  return (
    <Box sx={{ p: 4, maxWidth: 500, mx: "auto" }}>
      ...
      {step === 1 && (
        <Box>
          <Typography variant="subtitle1" sx={{ mb: 3, fontWeight: "bold", color: "#1976d2", display: 'flex', alignItems: 'center' }}>
            <ServerIcon sx={{ mr: 1 }} /> 1. KẾT NỐI SERVER (SSH)
          </Typography>

          <FormRow label="Server Name / IP" name="server" value={formData.server} onChange={handleChange} placeholder="192.168.1.100" />
          <FormRow label="SSH Port" name="sshPort" value={formData.sshPort} onChange={handleChange} placeholder="22" />
          <FormRow label="Username" name="user" value={formData.user} onChange={handleChange} />
          <FormRow label="Password" name="password" value={formData.password} onChange={handleChange} type="password" />

          {/* Dòng thông báo lỗi */}
          {errorMsg && (
            <Typography variant="caption" sx={{ color: "#d32f2f", mb: 2, display: "block", fontWeight: 'bold' }}>
              ⚠️ {errorMsg}
            </Typography>
          )}
      ...
      {step === 2 && (
        <Box>
          <Typography 
            variant="subtitle1" 
            sx={{ mb: 3, display: "flex", alignItems: "center", fontWeight: "bold", color: "#2e7d32" }}
          >
          ...
          <Box sx={{ display: "flex", gap: 2, mt: 2 }}>
            ...
            <Button 
              ...
              color="success" 
              startIcon={<LoginIcon />} 
              onClick={onConnectSuccess} // App.jsx sẽ dùng dbUser và dbPassword để test connection
            >
              Đăng nhập
            </Button>
          </Box>
