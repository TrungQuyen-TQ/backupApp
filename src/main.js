...
ipcMain.handle('create-sql-backup', async (event, dbConfig) => {
  try {
    ...
    const backupPath = path.join(app.getPath('downloads'), `local_backup.bak`);
    
    // Câu lệnh SQL để tạo file backup
    const query = `BACKUP DATABASE [${dbConfig.database}] TO DISK = '${backupPath}' WITH FORMAT, MEDIANAME = 'SQLServerBackup', NAME = 'Full Backup of ${dbConfig.database}';`;
